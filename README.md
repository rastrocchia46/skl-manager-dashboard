...
- I calciatori con Status `Fuori Listone` restano visibili nella rosa.
- La loro card è distinta con bordo tratteggiato e tonalità rosso/grigio.
- Viene mostrato il badge `Fuori Listone` accanto al nome.
- Le foto vengono leggermente desaturate per distinguere subito il loro stato.


## v79 - Fix selezione squadra durante aggiornamento live

- Corretto il problema per cui un aggiornamento Apps Script in corso poteva riportare la scheda alla squadra selezionata prima del cambio.
- La squadra attualmente scelta viene ora riletta al termine del refresh, quindi la selezione dell’utente non viene più sovrascritta.


## v80 - Palmarès storico persistente

- L'Albo d'oro non dipende più esclusivamente dalla sezione PREMI della stagione corrente.
- Inseriti nello storico i vincitori 2025/26: HYDRA SAVIANO (Campionato), AVRELIANA (Winter Cup), GISA SAVIANO (Kings Trophy).
- I futuri premi della stagione corrente continuano ad aggiungersi automaticamente quando vengono registrati nel gestionale.


## v81 - Totale tesserati

- La tabella **Rose per ruolo** ora mostra una sola colonna finale **Totale Tesserati**, calcolata direttamente dalle righe effettive della rosa.
- Rimosse le colonne ridondanti **Righe rosa** e **Riepilogo**.


## v82 - Fix dati nuova stagione e conteggi rose

- Il problema non era nel Google Sheet: il sito poteva ricadere sulla vecchia cache/fallback 2025/26.
- Nuova chiave cache dedicata alla stagione 2026/27 e rimozione della cache precedente.
- Timeout del caricamento live aumentato a 15 secondi per ridurre i fallback su mobile.
- `data.js` di fallback riallineato al gestionale `Inizio 2026-27`.
- I conteggi P/D/C/A e **Totale Tesserati** vengono calcolati direttamente dalla rosa ricevuta, non da aggregati eventualmente obsoleti.
- Aggiornata l'etichetta stagione in Home a **2026/27**.

## v83 - Sezione Storico

- Aggiunta una nuova voce **Storico** nel menu principale.
- Selettore stagione predisposto per più annate; prima stagione archiviata: **2025/26**.
- Classifica finale della stagione con tutte le squadre ordinate per piazzamento.
- Riepilogo dei vincitori di Campionato, Winter Cup e Kings Trophy.
- Selettore squadra per consultare:
  - movimenti di mercato storici (acquisti, prestiti e svincoli);
  - rosa della stagione divisa per ruolo;
  - valore in fantamilioni registrato per ciascun calciatore;
  - totale tesserati, numero movimenti e valore complessivo della rosa.
- I dati storici sono separati dai dati live tramite `history-data.js`, così i cambi di stagione del Google Sheet non cancellano l'archivio.

## v84 - Storico con punti e dettaglio operazioni

- Aggiunti i punti finali della classifica 2025/26 accanto a ogni squadra.
- Ogni movimento di mercato nello Storico è ora espandibile tramite il pulsante `+`.
- Il dettaglio mostra data, calciatore, squadra coinvolta, direzione/sessione e movimento contabile.
- Per i prestiti vengono mostrati anche formula contrattuale, condizioni, costo fisso e clausole/bonus quando registrati nel gestionale di chiusura 2025/26.
